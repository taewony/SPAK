...
SPAK enables a "Compounding" of engineering intelligence through a recursive five-step cycle. This ensures that every engineering task makes the next one easier.

1.  **Reverse Engineer:** Analyze existing high-performance implementations (e.g., NVIDIA's `attention.py`) to extract implicit **Design Axes** (Architectural choices) and **Tuning Parameters**.
2.  **Formalize & Generate:** Encode these axes into the Semiformal DSL. The Kernel then uses this specification to generate correct-by-construction executable artifacts (e.g., cuTile kernels).
3.  **Autotune & Trace:** Execute an **Engineering Loop** (Tuning Sweep) to find the optimal configuration for the specific hardware. Capture structured results as **TraceItems**.
4.  **Ingest & Crystallize:** Ingest the traces (including negative results) back into the DSL. Transform raw data into permanent **Knowledge** (Facts and Abductive Rules).
5.  **Reuse & Transfer:** Leverage the crystallized knowledge base to accelerate the design of the next kernel (e.g., transferring FMHA optimization laws to a Convolution kernel).
