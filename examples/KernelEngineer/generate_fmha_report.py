...
FMHA_DIR = os.path.join(BASE_DIR, "FMHA")

BENCHMARKS = [
    {
        "name": "Step 1: Python Prototype",
        "script": os.path.join(FMHA_DIR, "fmha_step1_python_ref.py"),
        "desc": "Verification of Online Softmax Invariant (NumPy).",
    },
    {
        "name": "Step 2: Naive Kernel",
        "script": os.path.join(FMHA_DIR, "fmha_step2_naive_kernel.py"),
        "desc": "Baseline kernel with global memory writes.",
    },
    {
        "name": "Step 3: Fused Kernel",
        "script": os.path.join(FMHA_DIR, "fmha_step3_fused_kernel.py"),
        "desc": "Fused Pipeline (Q-K-V) with Shared Memory.",
    },
    {
        "name": "Step 4: Auto-Tuned",
        "script": os.path.join(FMHA_DIR, "fmha_step4_autotuner.py"),
