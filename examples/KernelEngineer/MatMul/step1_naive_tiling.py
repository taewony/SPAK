...
def main():
    ...
    for _ in range(5): torch.matmul(t_A, t_B) # Warmup
    ...
    # 3. Benchmark Kernel
    grid = (16, 1, 1) # Low fixed launch count
    
    for _ in range(5): ct.launch(cp.cuda.get_current_stream(), grid, kernel, (d_A, d_B, d_C)) # Warmup
    ...
    print(f"{'Naive Tiling':<20} | {k_ms:<10.3f} | {k_tflops:<8.2f} | {pt_ms/k_ms:.2f}x")
    print("-" * 60)
    
    # Standard output for generator parser
    ...
    trace = {
        "type": "Performance",
        "step_name": "Level 1: Naive Tiling",
        "tflops": k_tflops,
        "speedup": pt_ms / k_ms
    }
