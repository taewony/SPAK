...
def main():
    ...
    # DSL Trace Emission (Best Result for Largest Size)
    # Assuming the last run (largest size) is the target for the report
    if 'spak_tf' in locals():
        import json
        trace = {
            "type": "Performance",
            "step_name": "Level 5: Auto-Tuned",
            "tflops": spak_tf,
            "speedup": (spak_tf / torch_tf) if torch_tf > 0 else 0.0
        }
